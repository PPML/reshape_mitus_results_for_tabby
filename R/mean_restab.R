...
mean_small_restabs <- function(restabs, nr, nints) { 
	age_id <- (2022:2050)-1949
	mResTabfb <- mResTabus <- mResTab <- array(NA,dim=c(nints,length(age_id),14,11))
...
mean_big_restabs <- function(restabs, nr, nints) { 
	...
	age_id <- (2022:2050)-1949
  mResTabfb <- mResTabus <- mResTab <- array(NA,dim=c(nints,length(age_id),14,4))
