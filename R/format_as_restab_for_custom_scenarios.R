  ...
  for (intv in 1:nintvs){
    ...
    for (b_ag in 1:4){
      ...
      # for (ag_vec in ag_vector){
        ...
        #percentage of ltbi prevalence
        ResTab[1:nr+((intv-1)*nr),,2,b_ag]<-apply(o[,age_id,c(54+ag_vec,65+ag_vec)],c(1,2),sum)/rowSums(o[, age_id,2+ag_vec], dims = 2)*1e2
        #incident tb infections per million
        ResTab[1:nr+((intv-1)*nr),,3,b_ag]<-apply(o[,age_id,c(564+ag_vec,575+ag_vec)],c(1,2),sum)/rowSums(o[, age_id,2+ag_vec], dims = 2)*1e6
        # TB notifications (alive+dead at diagnosis) per million
        ResTab[1:nr+((intv-1)*nr),,4,b_ag]<-apply(o[,age_id,c(135+ag_vec,188+ag_vec)],c(1,2),sum)/rowSums(o[, age_id,2+ag_vec], dims = 2)*1e6
        # tb attributable deaths per million
        ResTab[1:nr+((intv-1)*nr),,5,b_ag]<-apply(o[,age_id,c(87+ag_vec,98+ag_vec)],c(1,2),sum)/rowSums(o[, age_id,2+ag_vec], dims = 2)*1e6

        ################################################################################
        #US Born population
        ################################################################################
        ResTabus[1:nr+((intv-1)*nr),,1,b_ag]<-o[,age_id,1]/1e6
        # #percentage of ltbi prevalence
        ResTabus[1:nr+((intv-1)*nr),,2,b_ag]<-(rowSums(o[, age_id,54+ag_vec], dims = 2)/rowSums(o[, age_id,32+ag_vec], dims = 2))*1e2
        #incident tb infections per million
        ResTabus[1:nr+((intv-1)*nr),,3,b_ag]<-rowSums(o[, age_id,564+ag_vec], dims=2)/rowSums(o[, age_id,32+ag_vec], dims = 2)*1e6
        # # TB notifications (alive+dead at diagnosis) per million
        ResTabus[1:nr+((intv-1)*nr),,4,b_ag]<-apply(o[,age_id,c(204+ag_vec,215+ag_vec)],c(1,2),sum)/rowSums(o[, age_id,32+ag_vec], dims = 2)*1e6
        # #  attributable deaths per million
        ResTabus[1:nr+((intv-1)*nr),,5,b_ag]<-rowSums(o[, age_id,87+ag_vec], dims = 2)/rowSums(o[, age_id,32+ag_vec], dims = 2)*1e6
        #
        # ################################################################################
        # #non-US Born population
        # ################################################################################
        ResTabfb[1:nr+((intv-1)*nr),,1,b_ag]<-o[,age_id,1]/1e6
        # #percentage of ltbi prevalence
        ResTabfb[1:nr+((intv-1)*nr),,2,b_ag]<-rowSums(o[, age_id,65+ag_vec], dims=2)/rowSums(o[, age_id,43+ag_vec], dims = 2)*1e2
        #incident tb infections per million
        ResTabfb[1:nr+((intv-1)*nr),,3,b_ag]<-rowSums(o[, age_id,575+ag_vec], dims=2)/rowSums(o[, age_id,43+ag_vec], dims = 2)*1e6
        # TB notifications (alive+dead at diagnosis) per million
        ResTabfb[1:nr+((intv-1)*nr),,4,b_ag]<-(apply(o[,age_id,c(135+ag_vec,188+ag_vec)],c(1,2),sum)-apply(o[,age_id,c(204+ag_vec,215+ag_vec)],c(1,2),sum))/rowSums(o[, age_id,43+ag_vec], dims = 2)*1e6
        # tb attributable deaths per million
        ResTabfb[1:nr+((intv-1)*nr),,5,b_ag]<-rowSums(o[, age_id,98+ag_vec],dims = 2)/rowSums(o[, age_id,43+ag_vec], dims = 2)*1e6
      # }
    }
